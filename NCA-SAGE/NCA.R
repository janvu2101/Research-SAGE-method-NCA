...
View(df_private)
dim(df_private)
table(is.na(df$zorg_inputs))
table(is.na(df$zallops))
#Scatter plot
...
#NCA
nca_zallops<-nca_analysis(df,"zallops", "zpeople", ceilings=c("ce_fdh", "cr_fdh"), test.rep = 10000)
nca_output(nca_zallops, summaries = TRUE)
nca_output(nca_zallops, bottlenecks = TRUE, summaries = FALSE)
nca_zinputs<-nca_analysis(df,"zorg_inputs", "zpeople", ceilings=c("ce_fdh", "cr_fdh"), test.rep = 10000)
...
#Outliers:
nca_outliers(df, "zorg_inputs","zpeople", plotly = TRUE)
nca_outliers(df, "zallops","zpeople", plotly = TRUE)
